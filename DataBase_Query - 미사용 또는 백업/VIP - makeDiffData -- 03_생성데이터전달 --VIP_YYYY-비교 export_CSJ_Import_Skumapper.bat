@ECHO off
@echo ===================================================================================
@echo = VIP - makeDiffData -- 02_생성데이터전달 --Kaola_YYYY-비교 export_CSJ_Import_Skumapper -- MongDB Query Excuter By SukJun Choi.
@echo ===================================================================================
SET STARTTIME=%TIME%
...
::IMPORT
set importDb0=vip_%year%_continuity
...
call ..\..\Development-Binary-Windows\Binary\MongoDB\4.0.0\mongoexport --host 192.168.1.2 --port 55528 --username b2link --password qlxnfldzmdb2017_ --authenticationDatabase=admin --db "%exportDb0%" --collection "%exportCol1%" --out "%outFile1%" --jsonArray
call ..\..\Development-Binary-Windows\Binary\MongoDB\4.0.0\mongoexport --host 192.168.1.2 --port 55528 --username b2link --password qlxnfldzmdb2017_ --authenticationDatabase=admin --db "%exportDb0%" --collection "%exportCol3%" --out "%outFile3%" --jsonArray
call ..\..\Development-Binary-Windows\Binary\MongoDB\4.0.0\mongoexport --host 192.168.1.2 --port 55528 --username b2link --password qlxnfldzmdb2017_ --authenticationDatabase=admin --db "%exportDb0%" --collection "%exportCol2%" --out "%outFile2%" --jsonArray
call ..\..\Development-Binary-Windows\Binary\MongoDB\4.0.0\mongoexport --host 192.168.1.2 --port 55528 --username b2link --password qlxnfldzmdb2017_ --authenticationDatabase=admin --db "%exportDb0%" --collection "%exportCol4%" --out "%outFile4%" --jsonArray
