...
//----------------------------------------------------------------------------------------------------;
var fileNm = "000 - Taobao-Daily - daily_master.daily_master - check";
...
//----------------------------------------------------------------------------------------------------;
	...
	//Taobao-Daily - daily_master.daily_master - check;

	FUNCTIONS.push(function(){

		var fileNm = "000 - Taobao-Daily - daily_master.daily_master - check.dbjs";
		...
		//쿼리설정;
		var _query = SUtilFsReadStream.getFile( filePath ).toString();
		var query = _query;//.replace( "<!=TARGET_MONTH=!>", CONFIG.PARAMTERS_MONGODB_QUERY.TARGET_MONTH );
/*
	_to = {};
	_to.date = dateToFormat[ "YYYY-MM-DD" ]( d.date);
	_to.month_sold_quantity = d.month_sold_quantity;
	_to.Daily_Sales = d.Daily_Sales;
*/
		//쿼리실행;
		var info = { host : TARGET_DB_PORT[ "TAOBAO-DAILY" ].host, port : TARGET_DB_PORT[ "TAOBAO-DAILY" ].port, dbNm : "admin", query : query, cbComplete : function( error, result ){
				//debugger;
				//if( error ){ throw new Error(); return; }
				...
				for( z in result ){
					...
					zo = result[ z ]
					var _tmp = "date,month_sellCount,daily_sellCount\n";
					var i = 0, iLen = zo.length,io;
					for(;i<iLen;++i){
						io = zo[ i ];
						_tmp += io.date + "," + io.month_sold_quantity + "," + io.price_max + "\n"
					}

					SUtilFsWriteStream.writeFile( _CWD_ + "/" + z + ".csv", _tmp );

				}
				/*/
				var i = 0,iLen = result.length,io;
				var _tmp = "date,month_sellCount,daily_sellCount\n";
				for(;i<iLen;++i){
					io = result[ i ];
					_tmp += io.date + "," + io.goods_id + "," + io.month_sold_quantity + "," + io.price_max + "\n"
				}
				SUtilFsWriteStream.writeFile( _CWD_ + "/priceMaxCheck.csv", _tmp );
				//*/
