...
var fancygrid0_evt_click_btn = function( grid, value ){
	...
	var FNS = {
		"List Chart" : function( grid, value ){
			...
			window.SYS0037_UI_API.fancygrid.event.click_btn_render_brand_distribution__grid( grid, value, function( data ){
...
//				}
//
//				if( !_CTRLS.fancygrid_1 ) _CTRLS.fancygrid_1 = window.SYS0037_UI_API.fancygrid.render_brand_distribution__grid( DATA )
//				else
//				{
//					//update;
...
//				}
				...
				if( !_CTRLS.chart0 )
				{
					...
					_ELS.CHART0.style.height = ( _CTRLS.fancygrid_0.getHeight() / 2 ) + "px"

					DATA.element = _ELS[ id ];
					window.SYS0037_UI_API.echarts.merge_brand_by_keyword_Async( DATA,function(chart){
						_CTRLS.chart0 = chart
						_CTRLS.chart0.on('dblclick', function( data ){
							var nm_keyword;
							if( data.componentType == "xAxis" ) nm_keyword = data.value;
							if( data.componentType == "series" ) nm_keyword = data.name;
...
//								, COL : value.value.COL
							...
							if( !_CTRLS.chart2 )
							{
								...
								_ELS.CHART2.appendChild( _ELS[ id ] );
								_ELS.CHART2.style.height = ( _CTRLS.fancygrid_0.getHeight() / 2 ) + "px"

								DATA.element = _ELS[ id ];
								window.SYS0037_UI_API.echarts.merge_brand_by_keyword_trend_Async( DATA,function(chart){
									_CTRLS.chart2 = chart
								});
							}
							else
							{
								window.SYS0037_UI_API.echarts.merge_brand_by_keyword_trend_Async.updateSeriesData( _CTRLS.chart2, DATA,function(chart){
									_CTRLS.chart2 = chart
								});
							}
				...
				{
					window.SYS0037_UI_API.echarts.merge_brand_by_keyword_Async.updateSeriesData( _CTRLS.chart0, DATA,function(chart){
						_CTRLS.chart0 = chart
					});
				}
...
//
//				DATA.element = _ELS[ id ];
//				 window.SYS0037_UI_API.echarts.brand_info_view_Async( DATA,function(chart){
//					_CTRLS.chart1 = chart
//				});
//			}
//			else
//			{
//				window.SYS0037_UI_API.echarts.brand_info_view_Async.updateSeriesData( _CTRLS.chart1, DATA,function(chart){
//					_CTRLS.chart1 = chart
//				});
//			}
		}
