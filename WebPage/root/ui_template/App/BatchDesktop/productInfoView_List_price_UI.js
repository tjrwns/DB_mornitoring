...
//	, DIV_SEARCH_CONDITION_CONTAINER : F0( window.document, "id-search-condition" )
...
var _초기화면렌더링 = function()
{
	...
	_CTRLS.fancygrid_0.on( "cellclick", function( grid, value ){
		...
		_CTRLS.window0 = window.SYS0037_UI_API.dhtmlx.createWindow__attachLayout( t0.DB + "." + t0.COL, "2U" )
		...
		//기존돔에 서치관련 돔을 생성한다.;
		var el_search = window.SYS0037_UI_API.fancygrid.render_product_list_thumnail.make_search_input_horizontal();

		//_ELS.DIV_FANCYGRID_1_CONTAINER.appendChild( el_search );
		_CTRLS.window0.dataObj.items[ 0 ].attachObject( el_search );
		_CTRLS.window0.dataObj.items[ 0 ].setWidth( window.innerWidth * 0.1 );
		_CTRLS.window0.dataObj.items[ 0 ].setText( "Product Price Trend" );
		...
		//윈도우레이아웃설정 우측;
		//_CTRLS.window0.dataObj.items[ 1 ].setWidth( window.innerWidth * 0.9 );
		//_CTRLS.window0.dataObj.items[ 1 ].setHeight( window.innerHeight * 0.8 );
		_CTRLS.window0.dataObj.items[ 1 ].attachObject( _ELS.DIV_FANCYGRID_LIST );
		//_CTRLS.window0.dataObj.items[ 1 ].showInnerScroll();

		_CTRLS.window0.dataObj.items[ 1 ].setText( "Product Info" );
		...
		var DATA = {
			...
			, width : "100%"
			, height : _CTRLS.window0.dataObj.items[1]._getHeight() - _CTRLS.window0.dataObj.items[1]._getHdrHeight()
