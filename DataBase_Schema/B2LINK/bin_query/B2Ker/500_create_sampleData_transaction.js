...
var create_transaction_entity = function( d )
{
	...
	var doc = createDoc( "transaction", "transaction_entity" );
		...
		doc.cd$category_transaction_result = "CTR003"; //거래 상태 Category Transaction Result ID (FK); // string
		...
		doc.transaction_currency = { "_id$currency" : NumberInt( 34 ), "cd$currency" : "CNY", "price" : 1100 }
...
var create_transaction_quote_purchase_req = function( g_cd_transaction_entity, transaction_quote_sale_req, d )
{
	...
	var doc = createDoc( "transaction_quote_purchase", "transaction_quote_purchase_req" );
		...
		doc._ids$member = d._ids$member_register_modifier;
		...
		doc.items = clone( transaction_quote_sale_req.items ); //가견적 요청 품목 명세서 - 브랜드 명, 상품 명, 수량, 단위 등의 내용을 포함한다.;
...
var create_transaction_quote_purchase_res = function( g_cd_transaction_entity, transaction_quote_purchase_req, d )
{
	...
	var doc = createDoc( "transaction_quote_purchase", "transaction_quote_purchase_res" );
		...
		doc.items = clone( transaction_quote_purchase_req.items ); //가견적 요청 품목 명세서 - 브랜드 명, 상품 명, 수량, 단위 등의 내용을 포함한다.;
	...
	for( ; i<iLen; ++i )
	{
		doc.items[ i ].price_supply_per_item = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 70 };
		doc.items[ i ].price_supply_quantity = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 70 };
		doc.items[ i ].tax_per_quantity = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 30 };
		doc.items[ i ].tax_per_item = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 30 };
	}
...
var create_transaction_quote_sale_res = function( g_cd_transaction_entity, transaction_quote_sale_req, d )
{
	...
	var doc = createDoc( "transaction_quote_sale", "transaction_quote_sale_res" );
		...
		doc.items = clone( transaction_quote_sale_req.items );
	...
	for( ; i<iLen; ++i )
	{
		...
		doc.items[ i ].price_supply_per_quantity = {
			cost_logistics_wearing : { // 입고물류 비용(상세 비용을 합한 가격) 세액 제외;
				_id$currency : NumberInt( 79 ), cd$currency : "KRW"
				, price : 10
			}
 ...
 * @param {Object} d
 */
var create_transaction_schedule_wearing = function( g_cd_transaction_entity, transaction_purchase_order_purchase_res, d )
{
	...
	var doc = createDoc( "transaction", "transaction_schedule_wearing" );
		...
		doc.cd$transaction_entity = g_cd_transaction_entity;
		doc.cd$transaction_purchase_order_purchase_res = transaction_purchase_order_purchase_res.cd; // 구매발주확정서 코드;

		doc.cd$transaction_stock_wearing_res = null; // 입고증 코드;

		doc.date_predict_wearing = transaction_purchase_order_purchase_res.date_stock_wearing; // 입고 예정일자 (구매발주확정서); //object
		doc.date_wearing = null; // 입고 일자 (입고증 작성 일자); //object

		doc.date_predict_inspection = transaction_purchase_order_purchase_res.date_stock_wearing; // 검수 검품 작업 예정일자 (구매 발주 확정서상의 입고 일자); //object
		doc.date_inspection = null; // 실제 검수/검품 작업 시행일 (검수/검품 보고서 작성일); //object

		doc.date_predict_arrive = transaction_purchase_order_purchase_res.date_stock_wearing; // 창고 도착 예정일자 (구매 발주 확정서상의 입고 일자); //object
...
var create_transaction_cost_logistics_wearing = function( g_cd_transaction_entity, transaction_purchase_order_purchase_res, transaction_stock_wearing_res, d )
{
	...
	var doc = createDoc( "transaction", "transaction_cost_logistics_wearing" );
		...
		doc.cost_inspection_quality = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 100 }; // 검품비;
		doc.cost_inspection_count = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 0 };// 검수비;
		doc.cost_material = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 200 }; // 부자재비;
		doc.cost_operation = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 100 };// 작업비 (부자재비 제외);
		doc.tax_inspection_count = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 0 };// 검수 세액;
		doc.tax_inspection_quality = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 100 };// 검품 세액;
		doc.tax_material = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 10 };// 부자재 세액;
		doc.tax_operation = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 10 };// 작업비 세액;
		doc.cost_total = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 400 }; // 입고비용전체;
		doc.tax_total = { _id$currency : NumberInt( 79 ), cd$currency : "KRW", price : 130 };// 입고 세액 전체;
...
function create_transaction_quotation_sale_req( g_cd_transaction_entity, transaction_quote_sale_res, d )
{
	...
	var doc = createDoc( "transaction_quotation_sale", "transaction_quotation_sale_req" );
		...
		doc.items = clone( transaction_quote_sale_res.items ); //견적 요청 품목 명세서 - 브랜드 명, 상품 명, 수량, 단위 등의 내용을 포함한다.; // array
...
var create_transaction_quotation_sale_res = function( g_cd_transaction_entity, transaction_quotation_sale_req, d )
{
	...
	var doc = createDoc( "transaction_quotation_sale", "transaction_quotation_sale_res" );
		...
		doc.items = clone( transaction_quotation_sale_req.items ); //견적 품목 명세서 - 브랜드 명, 상품 명, 수량, 단위 등의 내용을 포함한다.; // array
...
var create_transaction_purchase_order_sale_req = function( g_cd_transaction_entity, transaction_quotation_sale_res, d )
{
	...
	var doc = createDoc( "transaction_purchase_order_sale", "transaction_purchase_order_sale_req" );
		...
		doc.items = clone( transaction_quotation_sale_res.items ); //발주 품목 명세서 - 브랜드 명, 상품 명, 수량, 단위 등의 내용을 포함한다.; // array
 ...
 * @param {Object} d
 */
var create_transaction_purchase_order_sale_res = function( g_cd_transaction_entity, transaction_purchase_order_purchase_res, d )
{
	...
	var doc = createDoc( "transaction_purchase_order_sale", "transaction_purchase_order_sale_res" );
		...
		doc.cd$transaction_purchase_order_sale_req = null; //판매 발주서 문서번호; // string
		doc.cd$transaction_entity = g_cd_transaction_entity; //Transaction_entity Code(FK) 거래식별코드; // string
		...
		doc.date_expire = date_expire; //Cancel Expire 문서 유효기간 ( 해당 문서의 유효기간( 해당 문서는 당 기간까지 유효함 )); // object
		doc.date_due = date_expire; //납기요청일 ( 물품이 납기되는 제한기간 ); // object
		doc.date_leadTime = getLeadTime( date_expire, doc.date_create ); //해당 판매 가견적 거래의 수행 기간 ( date_end_transaction - date_create_transaction ); // object
		doc.date_stock_release =d.date_stock_release;
		doc.items = clone( transaction_purchase_order_purchase_res.items ); //발주 품목 명세서 - 브랜드 명, 상품 명, 수량, 단위 등의 내용을 포함한다.; // array
		doc.price_total = {};
		doc.price_total.price = 0;
		doc.items.forEach ( function (item ) {


				var _id$currency = item.price_supply_per_quantity._id$currency;
				var price = item.price_supply_per_quantity.price / 3;
				delete item.price_supply_per_quantity;
				delete item.price_supply_per_item;
				item.price_supply_per_quantity = {};
				item.price_supply_per_quantity.cost_logistics_wearing = {};
				item.price_supply_per_quantity.cost_logistics_wearing._id$currency = NumberInt( _id$currency );
				item.price_supply_per_quantity.cost_logistics_wearing.cd$currency = "KRW";
				item.price_supply_per_quantity.cost_logistics_wearing.price = price  / 1.1;;
				item.price_supply_per_quantity.cost_logistics_wearing_tax = {};
				item.price_supply_per_quantity.cost_logistics_wearing_tax._id$currency = NumberInt( _id$currency );
				item.price_supply_per_quantity.cost_logistics_wearing_tax.cd$currency = "KRW";
				item.price_supply_per_quantity.cost_logistics_wearing_tax.price = price * 0.1
				item.price_supply_per_quantity.cost_logistics_release = {};
				item.price_supply_per_quantity.cost_logistics_release._id$currency = NumberInt( _id$currency );
				item.price_supply_per_quantity.cost_logistics_release.cd$currency = "KRW";
				item.price_supply_per_quantity.cost_logistics_release.price = price / 1.1;
				item.price_supply_per_quantity.cost_logistics_release_tax = {};
				item.price_supply_per_quantity.cost_logistics_release_tax._id$currency = NumberInt( _id$currency );
				item.price_supply_per_quantity.cost_logistics_release_tax.cd$currency = "KRW";
				item.price_supply_per_quantity.cost_logistics_release_tax.price = price * 0.1;
				item.price_supply_per_quantity.cost_purchase = {};
				item.price_supply_per_quantity.cost_purchase._id$currency = NumberInt( _id$currency );
				item.price_supply_per_quantity.cost_purchase.cd$currency = "KRW";
				item.price_supply_per_quantity.cost_purchase.price = price / 1.1;
				item.price_supply_per_quantity.cost_purchase_tax = {};
				item.price_supply_per_quantity.cost_purchase_tax._id$currency = NumberInt( _id$currency );
				item.price_supply_per_quantity.cost_purchase_tax.cd$currency = "KRW";
				item.price_supply_per_quantity.cost_purchase_tax.price = price * 0.1;
				item.price_supply_per_quantity.margin = 1.1;
				item.price_supply_per_quantity.price = {};
				item.price_supply_per_quantity.price._id$currency = NumberInt( 79 );
				item.price_supply_per_quantity.price.cd$currency = NumberInt( 79 );
				item.price_supply_per_quantity.price.price = ( item.price_supply_per_quantity.cost_logistics_wearing.price
					+ item.price_supply_per_quantity.cost_logistics_wearing_tax.price
					+ item.price_supply_per_quantity.cost_logistics_release.price
					+ item.price_supply_per_quantity.cost_logistics_release_tax.price
					+ item.price_supply_per_quantity.cost_purchase.price
					+ item.price_supply_per_quantity.cost_purchase_tax.price ) *  item.price_supply_per_quantity.margin;

				item.price_supply_per_item = {};
				item.price_supply_per_item.cost_logistics_wearing = {};
				item.price_supply_per_item.cost_logistics_wearing._id$currency = NumberInt( _id$currency );
				item.price_supply_per_item.cost_logistics_wearing.cd$currency = "KRW";
				item.price_supply_per_item.cost_logistics_wearing.price = item.price_supply_per_quantity.cost_logistics_wearing.price * item.quantity ;
				item.price_supply_per_item.cost_logistics_wearing_tax = {};
				item.price_supply_per_item.cost_logistics_wearing_tax._id$currency = NumberInt( _id$currency );
				item.price_supply_per_item.cost_logistics_wearing_tax.cd$currency = "KRW";
				item.price_supply_per_item.cost_logistics_wearing_tax.price = item.price_supply_per_quantity.cost_logistics_wearing_tax.price * item.quantity;
				item.price_supply_per_item.cost_logistics_release = {};
				item.price_supply_per_item.cost_logistics_release._id$currency = NumberInt( _id$currency );
				item.price_supply_per_item.cost_logistics_release.cd$currency = "KRW";
				item.price_supply_per_item.cost_logistics_release.price = item.price_supply_per_quantity.cost_logistics_release.price * item.quantity;
				item.price_supply_per_item.cost_logistics_release_tax = {};
				item.price_supply_per_item.cost_logistics_release_tax._id$currency = NumberInt( _id$currency );
				item.price_supply_per_item.cost_logistics_release_tax.cd$currency = "KRW";
				item.price_supply_per_item.cost_logistics_release_tax.price = item.price_supply_per_quantity.cost_logistics_release_tax.price * item.quantity;
				item.price_supply_per_item.cost_purchase = {};
				item.price_supply_per_item.cost_purchase._id$currency = NumberInt( _id$currency );
				item.price_supply_per_item.cost_purchase.cd$currency = "KRW";
				item.price_supply_per_item.cost_purchase.price = item.price_supply_per_quantity.cost_purchase.price * item.quantity;
				item.price_supply_per_item.cost_purchase_tax = {};
				item.price_supply_per_item.cost_purchase_tax._id$currency = NumberInt( _id$currency );
				item.price_supply_per_item.cost_purchase_tax.cd$currency = "KRW";
				item.price_supply_per_item.cost_purchase_tax.price = item.price_supply_per_quantity.cost_purchase_tax.price * item.quantity;
				item.price_supply_per_item.margin = 1.1;
				item.price_supply_per_item.price = {};
				item.price_supply_per_item.price._id$currency = NumberInt( _id$currency );
				item.price_supply_per_item.price.cd$currency = "KRW";
				item.price_supply_per_item.price.price = item.price_supply_per_quantity.price.price * item.quantity;

				doc.price_total.price += item.price_supply_per_item.price.price;
				doc.price_total._id$currency = NumberInt( _id$currency );
		}
	);
	...
	{
		//doc.items[ i ].info_delivery = { cd$cateogry_delivery_type : d.cd$category_delivery_type, info_delivery_option : null };
		doc.items[ i ].date_warehousing = date_expire;
	}
	...
	create_transaction_schedule_release( g_cd_transaction_entity, doc, d );
 ...
 * @param {Object} d
 */
var create_transaction_schedule_release = function( g_cd_transaction_entity, transaction_purchase_order_sale_res, d )
{
	print( "------------------------------ [ S ] create_transaction_schedule_release" );
	var doc = createDoc( "transaction", "transaction_schedule_release" );
		doc._id = NumberInt( db.getSiblingDB( "transaction" ).getCollection( "transaction_schedule_release" ).count() ); //Unique Key, Primary Key(PK); // uint
		doc._ids$member = d._ids$member_register_modifier;

		doc.cd = getCodeByCollection( "transaction", "transaction_schedule_wearing", { date_create : getArrayDateFromNewDate_forCd(), _id$partner_supplier : transaction_purchase_order_sale_res._id$partner_buyer - 0 }); //Unique Key;
		doc.cd$transaction_entity = g_cd_transaction_entity;
		doc.cd$transaction_purchase_order_sale_res = transaction_purchase_order_sale_res.cd;

		doc.date_predict_inspection = transaction_purchase_order_sale_res.date_stock_release; // 검수,검품 예정일자(판매 발주 확정서상의 출고 일자);
		doc.date_predict_packing_operation = transaction_purchase_order_sale_res.date_stock_release; // 패킹적업 예정일자 (판매 발주 확정서상의 출고 일자);
		doc.date_predict_release = transaction_purchase_order_sale_res.date_stock_release; // 출고 예정일자 (판매 발주 확정서상의 출고 일자);
	insertDoc( "transaction", "transaction_schedule_release", doc );
	print( "------------------------------ [ E ] create_transaction_schedule_release" );
};
...
var create_transaction_quotation_purchase_req = function( g_cd_transaction_entity, transaction_quotation_sale_req, d )
{
	...
	var date_expire = getObjectDateFromNewDate();
		...
		doc._ids$member = d._ids$member_register_modifier; //ID(FK) - 해당 컬랙션에서 회사 구성원 정보(이름, 연락처, 부서) 를 참조한다.; // uint
		...
		doc.items = clone( transaction_quotation_sale_req.items ); //가견적 요청 품목 명세서 - 브랜드 명, 상품 명, 수량, 단위 등의 내용을 포함한다.; // array
...
var create_transaction_quotation_purchase_res = function( g_cd_transaction_entity, transaction_quotation_purchase_req, d )
{
	...
	var doc = createDoc( "transaction_quotation_purchase", "transaction_quotation_purchase_res" );
		...
		doc.items = clone( transaction_quotation_purchase_req.items ); //구매 견적 요청 품목 명세서 - 브랜드 명, 상품 명, 수량, 단위, 상품가격정보, 물류스케쥴 등의 내용을 포함한다.; // array
...
var create_transaction_purchase_order_purchase_req = function( g_cd_transaction_entity, transaction_quotation_purchase_res, d )
{
	...
	var doc = createDoc( "transaction_purchase_order_purchase", "transaction_purchase_order_purchase_req" );
		...
		doc.items = [
			{
				...
				, _id$product_basic : NumberInt( 0 )
				, quantity : NumberInt( 100 )
				, UOM : { nm : "pkg", quantity : NumberInt( 100 ) }
			}
			, {
				_id$brand_basic : NumberInt( 0 )
				, _id$product_basic : NumberInt( 0 )
		...
		doc.info_delivery_option = d.info_delivery_option; //배송에 따라 필요한 정보를 추가 한다.; // array
		doc.memo = d.memo ? d.memo : null; //구매 발주에 대한 메모입력; // string
...
var create_transaction_purchase_order_purchase_res = function( g_cd_transaction_entity, transaction_purchase_order_purchase_req, d )
{
	...
	var doc = createDoc( "transaction_purchase_order_purchase", "transaction_purchase_order_purchase_res" );
		...
		doc.items = clone( transaction_purchase_order_purchase_req.items ); //구매 발주 회신 품목 명세서 - 브랜드 명, 상품 명, 수량, 단위, 세금정보, 물류스케쥴; // array
		doc.info_delivery_option = d.info_delivery_option; //배송에 따라 필요한 정보를 추가 한다.; // array
	...
	// 입고스케줄 생성;
	create_transaction_schedule_wearing( g_cd_transaction_entity, doc, d );
...
var create_transaction_stock_wearing_req = function( g_cd_transaction_entity, transaction_purchase_order_purchase_res, d )
{
	...
	var doc = createDoc( "transaction_stock_wearing", "transaction_stock_wearing_req" );
		...
		doc._id$company_info_basic = d._id$company_info_basic; //본사 정보를 저장하는 컬랙션으로 본사의 업체명, 주소, 전화번호, 팩스번호등을 참조한다; // uint
		doc._ids$member = d._ids$member_manager_register_modifier; //자사 member ID collection; // object
		doc._id$partner_supplier = d._id$partner_supplier; //Partner Supplier ID(FK) - 파트너사 - 상품공급자의 정보를 저장하는 컬랙션으로 업체명, 주소, 전화번호, 팩스번호등을 참조한다.; // uint
		doc._id$partner_person = d._id$partner_person; //파트너사 담당자 ID(FK); // uint
		...
		doc.items = clone( transaction_purchase_order_purchase_res.items ); //입고지시 품목 명세서 - 브랜드 명, 상품 명, 수량, 단위; // array
	...
	var schedule_wearing = targetCol.findOne({
			cd$transaction_entity : g_cd_transaction_entity
		...
		targetCol.update({
			...
			, { $set : {
				cd$transaction_stock_wearing_req : doc.cd
				, date_arrive : doc.date_request
				, date_leadTime_arrive : getLeadTime( schedule_wearing.date_predict_arrive, doc.date_request )
			}
		});
...
var create_transaction_packing_list_wearing = function( g_cd_transaction_entity, transaction_stock_wearing_req, d )
{
	...
	var doc = createDoc( "transaction_stock_wearing", "transaction_packing_list_wearing" );
		...
		doc._ids$member = d._ids$member_manager_register_modifier; // ID(FK) - 해당 컬랙션에서 회사 구성원 정보(이름, 연락처, 부서) 를 참조한다.; // uint
		...
		doc.items = clone( transaction_stock_wearing_req.items ); //패킹리스트 상의 아이템 정보 - 상품정보, UPC, 수량정보, 유통기한정보 등을 포함한다.; // array
...
var create_transaction_inspection_report_wearing = function( g_cd_transaction_entity, transaction_purchase_order_purchase_res, transaction_stock_wearing_req, d )
{
	...
	var doc = createDoc( "transaction_stock_wearing", "transaction_inspection_report_wearing" );
		...
		doc.items = clone( transaction_stock_wearing_req.items ); //검품검수 보고서의 상품정보; // array
	...
	var schedule_wearing = targetCol.findOne({
		...
		targetCol.update({
			...
			, { $set : {
				cd$transaction_inspection_report_wearing : doc.cd
				, date_inspection : doc.date_inspection
				, date_leadTime_inspection : getLeadTime( schedule_wearing.date_predict_inspection, doc.date_inspection )
			}
		});
...
var create_transaction_stock_wearing_res = function( g_cd_transaction_entity, transaction_stock_wearing_req, transaction_packing_list_wearing, transaction_inspection_report_wearing, d )
{
	...
	var doc = createDoc( "transaction_stock_wearing", "transaction_stock_wearing_res" );
		...
		doc.items = clone( transaction_stock_wearing_req.items ); //입고증 품목 명세서 - 브랜드 명, 상품 명, 수량, 단위, 세금정보, 물류스케쥴 등의 내용을 포함한다; // array
	...
	for( ; i<iLen; ++i )
	{
		...
		j=0;
		jLen=transaction_inspection_report_wearing.items[i].expiration_date_basic_info.length;
		for( ; j<jLen; ++j )
		{
			doc.items[ i ].expiration_dates.push({
				expiration_date : transaction_inspection_report_wearing.items[i].expiration_date_basic_info[j].expiration_date
				, location_lot : "AB0000000000" // 창고위치정보;
				, location_bin : "AB0000000000" // 창고위치정보;
				, quantity : transaction_inspection_report_wearing.items[i].expiration_date_basic_info[j].quantity
			});
		}
	...
	var schedule_wearing = targetCol.findOne({
			cd$transaction_entity : g_cd_transaction_entity
			, cd$transaction_stock_wearing_req : transaction_stock_wearing_req.cd
		});
		...
		targetCol.update({
			...
			, { $set : {
				cd$transaction_stock_wearing_res : doc.cd
				, date_wearing : getObjectDateFromNewDate()
				, date_leadTime_wearing : getLeadTime( schedule_wearing.date_predict_wearing, getObjectDateFromNewDate() )
			}
		});
...
var create_stock_entities = function( g_cd_transaction_entity, transaction_stock_wearing_res, d )
{
	...
	var items = clone( transaction_stock_wearing_res.items ); // 입고증 아이템 복사;
	var item = null;
	var d_entity = null;
	var d_quantity = null;
	...
	{
		item = items[ i ];
		j=0; jLen=item.expiration_dates.length;
		for( ; j<jLen; ++j )
		{
			d_entity = {};
			...
			d_entity.date_expire = item.expiration_dates[j].expiration_date;
			d_entity.lot = item.expiration_dates[j].location_lot;
			d_entity.price_per_quantity = transaction_purchase_order_purchase_res.items[ i ]["price_supply_per_quantity"];
			d_entity.cd$transaction_stock_wearing_res = transaction_stock_wearing_res.cd;
			d_entity.UPC = transaction_inspection_report_wearing.items[ i ].UPC;
			...
			d_quantity._id$member_public = 0;
			d_quantity.asset = { // 현재 SKU(Stock Entity CODE) 의 총 재고 자산; //object;
				_id$currency : NumberInt( transaction_purchase_order_purchase_res.items[ i ]["price_supply_per_quantity"]._id$currency )
				, cd$currency : "KRW"
				, price : ( transaction_purchase_order_purchase_res.items[ i ]["price_supply_per_quantity"].price * transaction_purchase_order_purchase_res.items[ i ].quantity )
			};
			d_quantity.bin = item.expiration_dates[j].location_bin; // 해당 상품이 위치한 위치 번호 소분류 - 물류창고의 lot 번호 (확보 불가능 시 창고코드로 대체); //string;

			d_quantity.cd$warehouse = d.cd$warehouse; // 창고 ID(FK); //string;
			...
			d_quantity.quantity_current = NumberInt( transaction_inspection_report_wearing.items[ i ].inspection_pass_quantity ); // 현재 수량-( 입고 수량 - 출고 예정 수량 - 출고 수량 ); // uint; // 최초 입력 시에는 검수/검품 보고서 정상수량;
			d_quantity.quantity_predict_release = [] // 출고 예정 수량; // array; 현재 스키마에 Array 로 되어있음.
			d_quantity.quantity_predict_wearing = NumberInt( 0 ); // 입고 예정 수량; // uint;
			d_quantity.quantity_wearing = NumberInt( transaction_inspection_report_wearing.items[ i ].inspection_pass_quantity ); // 실제 입고 수량  - 입고증; // uint;
...
var create_stock_entity = function( g_cd_transaction_entity, d )
{
	...
	var doc = createDoc( "stock", "stock_entity" );
		...
		doc.price_per_quantity = d.price_per_quantity; // 재고 상품의 개당 단가(세액제외) - 구매 발주확정서에서 확보;
		doc.tax_per_quantity = { price : d.price_per_quantity.price / 1.1 , cd$currency : "KRW", _id$currency : 79 }//d.tax_per_quantity; // 재고 상품의 개당 단가(세액제외) - 구매 발주확정서에서 확보;
...
var create_stock_quantity = function( g_cd_transaction_entity, cd$stock_entity, d )
{
	...
	var doc = createDoc( "stock", "stock_quantity" );
		...
		doc._id$brand_basic = d._id$brand_basic; // Brand ID (FK) - Brand Collection 을 참조( 브랜드명 등을 참조한다. ) // uint;
		doc._id$product_basic = d._id$product_basic; // Product ID (FK) - Product Collection 을 참조( 상품명 등을 참조한다. ) // uint;
		doc._id$member_public = d._id$member_public;
		...
		doc.quantity_predict_wearing = d.quantity_predict_wearing; // 입고 예정 수량; // uint;
		doc.quantity_predict_release = d.quantity_predict_release; // 출고 예정 수량; // array;
...
var create_transaction_stock_release_req = function( g_cd_transaction_entity, transaction_purchase_order_sale_res, d )
{
	...
	// 입고증;
	var arr_transaction_stock_wearing_res = [];
	db.getSiblingDB( "transaction_stock_wearing" ).getCollection( "transaction_stock_wearing_res" ).find({ cd$transaction_entity : g_cd_transaction_entity }).forEach(function(doc){ arr_transaction_stock_wearing_res.push( doc ); });

	// SKU;
	var arr_stock_entity = [];
	var j=0, jLen=arr_transaction_stock_wearing_res.length;
	for( ; j<jLen; ++j )
	{
		db.getSiblingDB( "stock" ).getCollection( "stock_entity" ).find({ cd$transaction_stock_wearing_res : arr_transaction_stock_wearing_res[j].cd }).forEach(function(doc){ arr_stock_entity.push( doc ); });
	}
	...
	var doc = getSchemaCollectionProperties( "transaction_stock_release", "transaction_stock_release_req" );
		...
		doc.cd$transaction_schedule_release = db.getSiblingDB( "transaction" ).getCollection( "transaction_schedule_release" ).findOne({
			cd$transaction_entity : g_cd_transaction_entity
			, cd$transaction_purchase_order_sale_res : transaction_purchase_order_sale_res.cd
		}).cd;
		...
		//doc.cd$category_release_operation_packing_material = d.cd$category_release_operation_packing_material; //category_release_operation_packaging_material ID (FK) - 출고시 작업지시 - 작업 종류 category 의 category_release_operation_packaging_material 컬랙션을 참조; // string
		...
		doc.items = clone( transaction_purchase_order_sale_res.items ); //출고지시 품목 명세서 - SKU, 수량, 단위; // array
	...
	for( ; i<iLen; ++i )
	{
		if( arr_stock_entity.length > 0 )
		{
			doc.items[ i ].cd$stock_entity = arr_stock_entity[ i ].cd;
			doc.items[ i ].cd$stock_quantity = db.getSiblingDB( "stock" ).getCollection( "stock_quantity" ).findOne({ cd$stock_entity : arr_stock_entity[ i ].cd }).cd;
		}
		else
		{
			doc.items[ i ].cd$stock_entity = "SKU-2017-03-24-PB0-13-2";
			doc.items[ i ].cd$stock_quantity = "SKU-2017-03-24-PB0-1-1-Q1";
		}
		...
		if( transaction_stock_wearing_req )
		{
			...
			doc.items[ i ].quantity_to_check = transaction_stock_wearing_req.items[ i ].quantity_to_check;
		...
		delete doc.items[ i ].UOM.quantity;
		delete doc.items[ i ].UOM.nm;
		delete doc.items[ i ].UOM;
	}
...
/*/
	var targetCol = db.getSiblingDB( "transaction" ).getCollection( "transaction_schedule_release" );
	var schedule_release = targetCol.findOne({
			cd$transaction_entity : g_cd_transaction_entity
			, cd$transaction_purchase_order_sale_res : transaction_purchase_order_sale_res.cd
		});
		...
		targetCol.update({
			...
			, { $set : {
				cd$transaction_stock_release_req : doc.cd
			}
		});
//*/
	return doc;
...
var create_transaction_inspection_report_release = function( g_cd_transaction_entity, transaction_stock_release_req, d )
{
	...
	var doc = createDoc( "transaction_stock_release", "transaction_inspection_report_release" );
		...
		doc.cd$transaction_entity = g_cd_transaction_entity; //transaction_purchase_order_purchase_res ID(FK) - transaction_purchase_order_sale_res 컬랙션에서 ID를 참조한다.; // string
		...
		doc.items = clone( transaction_stock_release_req.items ); //출고지시서의 상품정보; // array
	...
	for( ; i<iLen; ++i )
	{
		...
		doc.items[ i ].cd$stock_entity = transaction_stock_release_req.items[ i ].cd$stock_entity;
		doc.items[ i ].cd$stock_quantity = transaction_stock_release_req.items[ i ].cd$stock_quantity;
		doc.items[ i ].UPC = db.getSiblingDB( "stock" ).getCollection( "stock_entity" ).findOne({ cd : doc.items[ i ].cd$stock_entity }).UPC;
		doc.items[ i ].inspection_quantity = NumberInt( transaction_stock_wearing_req.items[ i ].quantity_to_check ); // 입고지시서의 검수/검품수량;
		doc.items[ i ].faulty_info_total_quantity = NumberInt( 0 ); // 출고불량정보 - 총수량;
		doc.items[ i ].faulty_info_condition_quantity = NumberInt( 0 ); // 출고불량정보 - 상태불량;
		doc.items[ i ].faulty_info_img_quantity = NumberInt( 0 ); // 출고불량정보 - 품목확인불량;
		doc.items[ i ].faulty_info_upc_quantity = NumberInt( 0 ); // 출고불량정보 - 바코드 불일치;
		doc.items[ i ].expiration_date_check_quantity = NumberInt( transaction_stock_wearing_req.items[ i ].quantity_to_check ); // 검수검품수량;
		doc.items[ i ].expiration_date_check_base_date = transaction_stock_wearing_req.items[ i ].expiration_date_check_base_date; // 유통기한 확인 기준일;
		doc.items[ i ].expiration_date_basic_info = [{
			expiration_date : transaction_stock_wearing_req.items[ i ].expiration_date_check_base_date // 출고상품 유통기한 확인 기준일;
			, expiration_date_check : NumberInt( 1 ) // 출고상품 유통기한 통과 여부;
			, quantity : NumberInt( 1 ) // 출고예정수량;
		}];
		doc.items[ i ].inspection_pass_quantity = NumberInt( transaction_stock_wearing_req.items[ i ].quantity ); // 상품출고정보 - 정상;
		doc.items[ i ].inspection_faulty_quantity = NumberInt( 0 ); // 상품출고정보 - 불량;
		doc.items[ i ].cost_inspection = { // 해당 상품의 검수/검품비용;
			id$currency : NumberInt( 79 )
			, price : 10
		};
	...
	var schedule_release = targetCol.findOne({
			cd$transaction_entity : g_cd_transaction_entity
			, cd$transaction_purchase_order_sale_res : doc.cd$transaction_purchase_order_sale_res
		});
		...
		targetCol.update({
			...
			, { $set : {
					cd$transaction_inspection_report_release : doc.cd
					, date_inspection : getObjectDateFromNewDate()
					, date_leadTime_inspection : getLeadTime( schedule_release.date_predict_inspection, getObjectDateFromNewDate() )
			}
		});
...
var create_transaction_packing_list_release = function( g_cd_transaction_entity, transaction_inspection_report_release, d )
{
	...
	var doc = createDoc( "transaction_stock_release", "transaction_packing_list_release" );
		...
		doc.items = clone( transaction_inspection_report_release.items ); //출고지시 품목 명세서 - SKU, 수량, 단위; // array
	...
	for( ; i<iLen; ++i )
	{
		doc.items[ i ].packing_info = [{
			...
			, unit_quantity_per_package : Number( 5 )
			, quantity : Number( 10 )
			, package_net_weight : 100.0
			, package_gross_weight : 1000.0
		}];
		...
		delete doc.items[ i ].faulty_info_upc_quantity;
		delete doc.items[ i ].expiration_date_check_quantity;
		delete doc.items[ i ].expiration_date_basic_info;
		delete doc.items[ i ].inspection_pass_quantity;
		delete doc.items[ i ].inspection_faulty_quantity;
		delete doc.items[ i ].cost_inspection;
	...
	var schedule_release = targetCol.findOne({
			cd$transaction_entity : g_cd_transaction_entity
			, cd$transaction_inspection_report_release : transaction_inspection_report_release.cd
		});
		...
		targetCol.update({
			...
			, { $set : {
					date_packing_operation : doc.date_request
					, date_leadTime_packing_operation : getLeadTime( schedule_release.date_predict_packing_operation, doc.date_request )
			}
		});
...
var create_transaction_stock_release_res = function( g_cd_transaction_entity, transaction_packing_list_release, transaction_purchase_order_sale_res, transaction_inspection_report_release, d )
{
	...
	var doc = createDoc( "transaction_stock_release", "transaction_stock_release_res" );
		...
		doc.items = clone( transaction_packing_list_release.items ); //출고지시 품목 명세서 - SKU, 수량, 단위; // array
	...
	var schedule_release = targetCol.findOne({
		...
		targetCol.update({
			...
			, { $set : {
					cd$transaction_stock_release_res : doc.cd
					, date_release : getObjectDateFromNewDate()
					, date_leadTime_release : getLeadTime( schedule_release.date_predict_release, getObjectDateFromNewDate() )
			}
		});
...
var create_transaction_delivery_information = function( g_cd_transaction_entity, transaction_stock_release_res )
{
	...
	var doc = createDoc( "transaction", "transaction_delivery_information" );
		...
		doc.cd = getCodeByCollection( "transaction", "transaction_delivery_information", { "cd$transaction_entity" : g_cd_transaction_entity } ); //배송 문서번호 ( 외부에서 코드값으로 사용된다. 형식 : GIS YYYYMMDD PARTNER_ID Number); // string
		...
		doc.date_delivery_end = null; // 배송완료일자; // object
		doc.delivery_history = [
			{
				cd$category_delivery_status : "CDS003" // 출고;
				, _id$member_public : transaction_stock_release_res._ids$member.manager // 출고증 담당자;
				, date_arrive_location : getObjectDateFromNewDate()
				, date_create : getObjectDateFromNewDate()
				, nm_location : "창고"
			}
...
var updateDoc_transaction_entity_category_transaction_result = function( entityCd, targetColName, cd, targetDBName )
{
	...
	if( targetDBName && targetColName )
	{
		var resultcode = null;
		var doc$category_transaction_result = db.getSiblingDB( "category").getCollection( "category_transaction_result" ).findOne({ nm_db : targetDBName, nm_col : targetColName });

		if( doc$category_transaction_result )
		{
			resultcode = doc$category_transaction_result.cd;
			var targetCol = db.getSiblingDB( "transaction" ).getCollection( "transaction_entity" );
				targetCol.update(
					{ cd : entityCd }//search Object;
...
var clone = function(obj)
{
	...
	if ( obj.length ) {
		copy = new Array();
		var i=0, iLen=obj.length;
		for( ; i<iLen; ++i )
		{
			copy[ i ] = clone( obj[ i ] );
		}
	}
	else copy = Object.assign({}, obj);
	return copy;
};
...
var insert_value_transaction = {
	...
	, _ids$member_register_modifier : {
		register : NumberInt( 1 )
		, modifier : null
	}
	, _ids$member_manager_register_modifier : {
		manager : NumberInt( 1 )
	...
	, cd$category_release_operation_packing_material : "CROPM002"
	, price_supply_total : {
		price : 100
		, _id$currency : NumberInt( 79 ), cd$currency : "KRW"
	}
...
var main = function()
{
	...
	//*/
	//------------------------------;
	// 2017.03.14 - 가견적, 견적 단계 하지 않기로 함;

	// 구매발주.확정서 - 판매발주확정서 - 입고 - 재고 - 출고 - 배송 - 정산;
	//*/
	main_20170314_depth5_1(); // 구매발주서;
	main_20170314_depth5_2(); // 구매발주확정서;
	main_20170314_depth5_3(); // 판매발주확정서;
	main_20170314_depth6_1(); // 입고지시서;
	main_20170314_depth6_2(); // 입고패킹리스트;
	main_20170314_depth6_3(); // 입고검수/검품보고서;
	// 입고확인지시서;
	main_20170314_depth6_5(); // 입고증;
	main_20170314_depth6_6(); // 재고처리;
	main_20170314_depth8_1(); // 출고지시서;
	main_20170314_depth8_2(); // 출고검수/검품보고서;
	main_20170314_depth8_3(); // 출고패킹리스트;
	main_20170314_depth8_4(); // 출고증;
	main_20170314_depth9_1(); // 배송;
	/*/
	main_20170314_depth9_1(); // 배송;
	//*/
...
var main_20170314_depth5_1 = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );

	// 5-1. 구매발주서 --------------------------------------------------;
	var doc_transaction_purchase_order_purchase_req = create_transaction_purchase_order_purchase_req( doc_transaction_entity.cd, null, insert_value_transaction );

	updateDoc_transaction_entity_category_transaction_result( doc_transaction_entity.cd, "transaction_purchase_order_purchase_req", doc_transaction_purchase_order_purchase_req.cd, "transaction_purchase_order_purchase" );
};

var main_20170314_depth5_2 = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
...
var main_20170314_depth5_3 = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
	...
	// 5-4. 판매발주확정서 --------------------------------------------------;
	var doc_transaction_purchase_order_sale_res = create_transaction_purchase_order_sale_res( doc_transaction_entity.cd, doc_transaction_purchase_order_purchase_res, insert_value_transaction );
...
var main_20170314_depth6_1 = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
...
var main_20170314_depth6_2 = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
...
var main_20170314_depth6_3 = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
...
var main_20170314_depth6_5 = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
...
var main_20170314_depth6_6 = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
...
var main_20170314_depth8_1 = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
...
var main_20170314_depth8_2 = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
...
var main_20170314_depth8_3 = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
...
var main_20170314_depth8_4 = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
...
var main_20170314_depth9_1 = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
