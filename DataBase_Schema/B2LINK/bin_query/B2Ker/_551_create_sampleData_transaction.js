...
var create_transaction_quote_sale_req = function( g_cd_transaction_entity, transaction_entity, d )
{
	...
	var doc = createDoc( "transaction_quote_sale", "transaction_quote_sale_req" );
		...
		doc.items = transaction_entity.items ? clone( transaction_entity.items ) : [
		{
 ...
 * @param {Number} _id$partner_supplier
 * @param {Object} d
 */
var create_transaction_quote_purchase_req = function( g_cd_transaction_entity, transaction_quote_sale_req, _id$partner_supplier, d )
{
	...
	var doc = createDoc( "transaction_quote_purchase", "transaction_quote_purchase_req" );
		doc = setObjectFromParameter( doc, d );
		doc._id$partner_supplier = _id$partner_supplier;
...
var create_transaction_purchase_order_sale_res = function( g_cd_transaction_entity, transaction_purchase_order_sale_req, d )
{
	...
	var doc = createDoc( "transaction_purchase_order_sale", "transaction_purchase_order_sale_res" );
		...
		doc.items = clone( transaction_purchase_order_sale_req.items ); //발주 품목 명세서 - 브랜드 명, 상품 명, 수량, 단위 등의 내용을 포함한다.; // array
 ...
 * @param {Object} d
 */
var create_transaction_quotation_purchase_req = function( g_cd_transaction_entity, transaction_quotation_sale_req, _id$partner_supplier, d )
{
	...
	var date_expire = getObjectDateFromNewDate();
		...
		doc._id$member_public = d._id$member_public; //member_public ID(FK) - 해당 컬랙션에서 회사 구성원 정보(이름, 연락처, 부서) 를 참조한다.; // uint
		doc._id$partner_supplier = _id$partner_supplier; //Partner Supplier ID(FK) - 파트너사 - 상품공급자의 정보를 저장하는 컬랙션으로 업체명, 주소, 전화번호, 팩스번호등을 참조한다.; // uint
		doc._id$partner_person = d._id$partner_person; //파트너사 담당자 ID(FK); // uint
 ...
 * @param {Object} d
 */
var create_transaction_quotation_purchase_res = function( g_cd_transaction_entity, transaction_quotation_purchase_req, _id$partner_supplier, d )
{
	...
	var doc = createDoc( "transaction_quotation_purchase", "transaction_quotation_purchase_res" );
		...
		doc.date_stock_wearing = date_expire; //물류스케쥴 - 입고일자 ( 공급자가 출고한 상품이 구매자에게 전달되는 날짜 ); // object
...
var create_transaction_purchase_order_purchase_req = function( g_cd_transaction_entity, transaction_quotation_purchase_res, d )
{
	...
	var doc = createDoc( "transaction_purchase_order_purchase", "transaction_purchase_order_purchase_req" );
		...
		doc._id$company_info_basic = d._id$company_info_basic; //본사 정보를 저장하는 컬랙션으로 본사의 업체명, 주소, 전화번호, 팩스번호등을 참조한다; // uint
		doc._id$member_public = d._id$member_public; //member_public ID(FK) - 해당 컬랙션에서 회사 구성원 정보(이름, 연락처, 부서) 를 참조한다.; // uint
		doc._id$partner_supplier = transaction_quotation_purchase_res._id$partner_supplier; //Partner Supplier ID(FK) - 파트너사 - 상품공급자의 정보를 저장하는 컬랙션으로 업체명, 주소, 전화번호, 팩스번호등을 참조한다.; // uint
		doc._id$partner_person = d._id$partner_person; //파트너사 담당자 ID(FK); // uint
		doc._id$trade_terms = d._id$trade_terms; //trade_terms ID (FK) - trade_terms Collection 을 참조; // uint
		...
		doc.items = clone( transaction_quotation_purchase_res.items ); //구매 견적 요청 품목 명세서 - 브랜드 명, 상품 명, 수량, 단위 등의 내용을 포함한다.; // array
...
var create_transaction_purchase_order_purchase_res = function( g_cd_transaction_entity, transaction_purchase_order_purchase_req, d )
{
	...
	var doc = createDoc( "transaction_purchase_order_purchase", "transaction_purchase_order_purchase_res" );
		...
		doc._id$company_info_basic = d._id$company_info_basic; //본사 정보를 저장하는 컬랙션으로 본사의 업체명, 주소, 전화번호, 팩스번호등을 참조한다; // uint
		doc._id$member_public = d._id$member_public; //member_public ID(FK) - 해당 컬랙션에서 회사 구성원 정보(이름, 연락처, 부서) 를 참조한다.; // uint
		doc._id$partner_supplier = transaction_purchase_order_purchase_req._id$partner_supplier; //Partner Supplier ID(FK) - 파트너사 - 상품공급자의 정보를 저장하는 컬랙션으로 업체명, 주소, 전화번호, 팩스번호등을 참조한다.; // uint
		doc._id$partner_person = d._id$partner_person; //파트너사 담당자 ID(FK); // uint
		doc._id$trade_terms = d._id$trade_terms; //trade_terms ID (FK) - trade_terms Collection 을 참조; // uint
...
var create_transaction_stock_wearing_req = function( g_cd_transaction_entity, transaction_purchase_order_purchase_res, d )
{
	...
	var doc = createDoc( "transaction_stock_wearing", "transaction_stock_wearing_req" );
		...
		doc._id$partner_supplier = NumberInt( transaction_purchase_order_purchase_res._id$partner_supplier ); //Partner Supplier ID(FK) - 파트너사 - 상품공급자의 정보를 저장하는 컬랙션으로 업체명, 주소, 전화번호, 팩스번호등을 참조한다.; // uint
		doc._id$partner_person = d._id$partner_person; //파트너사 담당자 ID(FK); // uint
...
var create_transaction_packing_list_wearing = function( g_cd_transaction_entity, transaction_stock_wearing_req, d )
{
	...
	var doc = createDoc( "transaction_stock_wearing", "transaction_packing_list_wearing" );
		...
		doc.cd = getCodeByCollection( "transaction_stock_wearing", "transaction_packing_list_wearing", { date_create : getArrayDateFromNewDate_forCd(), _id$partner_supplier : transaction_stock_wearing_req._id$partner_supplier - 0 } ); //입고 패킹리스트 문서번호 ( 외부에서 코드값으로 사용된다. 형식 : OOO YYYYMMDD PARTNER_ID Number); // string
...
var create_transaction_inspection_report_wearing = function( g_cd_transaction_entity, transaction_purchase_order_purchase_res, transaction_stock_wearing_req, d )
{
	...
	var doc = createDoc( "transaction_stock_wearing", "transaction_inspection_report_wearing" );
		...
		doc.cd = getCodeByCollection( "transaction_stock_wearing", "transaction_inspection_report_wearing", { date_create : getArrayDateFromNewDate_forCd(), _id$partner_supplier : transaction_stock_wearing_req._id$partner_supplier - 0 } ); //입고 검수검품 보고서 문서번호 ( 외부에서 코드값으로 사용된다. 형식 : OOO YYYYMMDD PARTNER_ID Number); // string
...
var create_transaction_stock_wearing_res = function( g_cd_transaction_entity, transaction_stock_wearing_req, transaction_packing_list_wearing, transaction_inspection_report_wearing, d )
{
	...
	var doc = createDoc( "transaction_stock_wearing", "transaction_stock_wearing_res" );
		...
		doc._id$company_info_basic = d._id$company_info_basic; //본사 정보를 저장하는 컬랙션으로 본사의 업체명, 주소, 전화번호, 팩스번호등을 참조한다; // uint
		doc._id$partner_supplier = transaction_stock_wearing_req._id$partner_supplier; //Partner Supplier ID(FK) - 파트너사 - 상품공급자의 정보를 저장하는 컬랙션으로 업체명, 주소, 전화번호, 팩스번호등을 참조한다.; // uint
		doc._id$partner_person = d._id$partner_person; //파트너사 담당자 ID(FK); // uint
		doc._ids$member = d._ids$member; //자사 member ID collection; // object
 ...
 * @param {Document} transaction_stock_wearing_res 입고증
 * @param {Document} transaction_purchase_order_purchase_res 구매발주확정서
 * @param {Document} transaction_inspection_report_wearing 검수검품보고서
 * @param {Object} d
 */
var create_stock_entities = function( g_cd_transaction_entity, transaction_stock_wearing_res, transaction_purchase_order_purchase_res, transaction_inspection_report_wearing, d )
{
	print( "------------------------------ [ S ] create_stock_entities" );

	var items = clone( transaction_stock_wearing_res.items );
	...
	for( ; i<iLen; ++i )
	{
		...
		d_quantity.quantity_predict_release = NumberInt( 0 ); // 출고 예정 수량; // uint;
...
var create_transaction_inspection_report_release = function( g_cd_transaction_entity, transaction_stock_release_req, d )
{
	...
	var doc = createDoc( "transaction_stock_release", "transaction_inspection_report_release" );
		...
		doc.items = clone( transaction_stock_release_req.items ); //검품검수 보고서의 상품정보; // array
...
var setObjectFromParameter= function( o, d )
{
	...
	if( o.hasOwnProperty( "_id$partner_supplier" ) ) o[ "_id$partner_supplier" ] = d._id$partner_suppliers[ 0 ];
...
function clone(obj) {
  ...
  for (var attr in obj) {
	...
	if (typeof( copy[attr] ) == "number" ) {
		copy[attr] = NumberInt( copy[attr]);
	}
  }
  return copy;
}
...
{
	//*/
	main_depth1_1_n( 3 ); // 판매가견적 1회, 구매가견적 n회;
	main_depth1_1_nBrand(); // 판매가견적 1회, 구매가견적 nBrand;
	main_depth1_1_nBrand_n( 3 ); // 판매가견적 1회, 구매가견적 nBrand n회;
	main_depth1_n( 3 ); // 판매가견적 n회, 구매가견적 n회;
	main_depth1_n_nBrand_n( 3 ); // 판매가견적 n회, 구매가견적 nBrand n회;
	main_depth2_n_nBrand( 3 ); // 판매가견적 n회, 구매가견적 nBrand n회, 판매견적1회, 구매견적 nBrand;
	main_depth3_n_nBrand( 3 ); // 판매가견적 n회, 구매가견적 nBrand n회, 판매견적1회, 구매견적 nBrand, 판매발주&구매발주;
	main_depth4_n_nBrand( 3 ); // 판매가견적 n회, 구매가견적 nBrand n회, 판매견적1회, 구매견적 nBrand, 판매발주&구매발주, 입고 n회;
	main_depth5_n_nBrand( 3 ); // 판매가견적 n회, 구매가견적 nBrand n회, 판매견적1회, 구매견적 nBrand, 판매발주&구매발주, 입고 n회, 출고 n회;
	/*/
	main_depth5_n_nBrand( 3 ); // 판매가견적 n회, 구매가견적 nBrand n회, 판매견적1회, 구매견적 nBrand, 판매발주&구매발주, 입고 n회, 출고 n회;
	//*/
};
 ...
 * @function
 */
var main_depth1_1_n = function( n )
{
	// 0.거래 생성 --------------------------------------------------;
	...
	var doc_transaction_quote_purchase_req = null;
	var doc_transaction_quote_purchase_res = null;
	...
	for( ; i<iLen; ++i )
	{
		...
		{
			// 1-1-1. 구매가견적요청서 --------------------------------------------------;
			doc_transaction_quote_purchase_req = create_transaction_quote_purchase_req( doc_transaction_entity.cd, doc_transaction_quote_sale_req, insert_value_transaction._id$partner_suppliers[0], insert_value_transaction );
		}
		else
		{
			// 1-1-1. 구매가견적요청서 --------------------------------------------------;
			doc_transaction_quote_purchase_req = create_transaction_quote_purchase_req( doc_transaction_entity.cd, doc_transaction_quote_purchase_res, insert_value_transaction._id$partner_suppliers[0], insert_value_transaction );
		}

		// 1-1-2. 구매가견적회신서 --------------------------------------------------;
		doc_transaction_quote_purchase_res = create_transaction_quote_purchase_res( doc_transaction_entity.cd, doc_transaction_quote_purchase_req, insert_value_transaction );
	}
 ...
 * @function
 */
var main_depth1_1_nBrand = function()
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
	...
	var i=0, iLen=insert_value_transaction._id$partner_suppliers.length;
	for( ; i<iLen; ++i )
	{
		// 1-1-1. 구매가견적요청서 --------------------------------------------------;
		doc_transaction_quote_purchase_req = create_transaction_quote_purchase_req( doc_transaction_entity.cd, doc_transaction_quote_sale_req, insert_value_transaction._id$partner_suppliers[ i ], insert_value_transaction );
 ...
 * @function
 */
var main_depth1_1_nBrand_n = function( n )
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
	...
	var i=0, iLen=n;
	var j=0, jLen=insert_value_transaction._id$partner_suppliers.length;
	for( ; i<iLen; ++i )
	{
		for( ; j<jLen; ++j )
		{
			...
			{
				// 1-1-1. 구매가견적요청서 --------------------------------------------------;
				doc_transaction_quote_purchase_req = create_transaction_quote_purchase_req( doc_transaction_entity.cd, doc_transaction_quote_sale_req, insert_value_transaction._id$partner_suppliers[j], insert_value_transaction );
			}
			else
			{
				// 1-1-1. 구매가견적요청서 --------------------------------------------------;
				doc_transaction_quote_purchase_req = create_transaction_quote_purchase_req( doc_transaction_entity.cd, doc_transaction_quote_purchase_res, insert_value_transaction._id$partner_suppliers[j], insert_value_transaction );
			}
 ...
 * @function
 */
var main_depth1_n = function( n )
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );

	var doc_transaction_quote_sale_req = null;
	var doc_transaction_quote_purchase_req = null;
	var doc_transaction_quote_purchase_res = null;
	var doc_transaction_quote_sale_res = null;
	...
	for( ; i<iLen; ++i )
	{
		...
		{
			// 1-1. 판매 가견적 요청서 --------------------------------------------------;
			doc_transaction_quote_sale_req = create_transaction_quote_sale_req( doc_transaction_entity.cd, doc_transaction_entity, insert_value_transaction );
			...
			// 1-2.판매 가견적 회신서 --------------------------------------------------;
			doc_transaction_quote_sale_res = create_transaction_quote_sale_res( doc_transaction_entity.cd, doc_transaction_quote_sale_req, insert_value_transaction );
		}
		else
		{
			// 1-1. 판매 가견적 요청서 --------------------------------------------------;
			doc_transaction_quote_sale_req = create_transaction_quote_sale_req( doc_transaction_entity.cd, doc_transaction_quote_sale_res, insert_value_transaction );
...
/**
 * 판매가견적 n회, 구매가견적 nBrand n회
 */
var main_depth1_n_nBrand_n = function( n )
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
...
/**
 * 판매가견적 n회, 구매가견적 nBrand n회, 판매견적1회, 구매견적 nBrand
 */
var main_depth2_n_nBrand = function( n )
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
	...
	// 2-2. 구매견적요청서 --------------------------------------------------;
	var doc_transaction_quotation_purchase_req = null;
	var doc_transaction_quotation_purchase_res = null;
	j = 0;
	for( ; j<jLen; ++j )
	{
		db.getSiblingDB( "transaction_quote_purchase" ).getCollection( "transaction_quote_purchase_res" ).find({
			cd$transaction_entity : doc_transaction_entity.cd
			, _id$partner_supplier : insert_value_transaction._id$partner_suppliers[ j ]
		}).sort({ _id : -1 }).limit( 1 ).forEach(function( doc ){
			doc_transaction_quote_purchase_res = doc;
		});

		doc_transaction_quotation_purchase_req = create_transaction_quotation_purchase_req( doc_transaction_entity.cd, doc_transaction_quote_purchase_res, insert_value_transaction._id$partner_suppliers[j], insert_value_transaction );
		doc_transaction_quotation_purchase_res = create_transaction_quotation_purchase_res( doc_transaction_entity.cd, doc_transaction_quotation_purchase_req, insert_value_transaction._id$partner_suppliers[j], insert_value_transaction );
	}
...
/**
 * 판매가견적 n회, 구매가견적 nBrand n회, 판매견적1회, 구매견적 nBrand, 판매발주&구매발주;
 */
var main_depth3_n_nBrand = function( n )
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
	...
	// 구매견적회신서로부터 정보를 가지고 옴;
	var transaction_quotation_purchase_res = [];
	db.getSiblingDB( "transaction_quotation_purchase" ).getCollection( "transaction_quotation_purchase_res" ).find({
		cd$transaction_entity : doc_transaction_entity.cd
	}).sort({ _id : 1 }).forEach(function( doc ){
		transaction_quotation_purchase_res.push( doc );
	});
	i=0, iLen=transaction_quotation_purchase_res.length;
	var doc_transaction_purchase_order_purchase_req = null;
	var doc_transaction_purchase_order_purchase_res = null;
	for( ; i<iLen; ++i )
	{
		// 구매발주서;
		doc_transaction_purchase_order_purchase_req = create_transaction_purchase_order_purchase_req( doc_transaction_entity.cd, transaction_quotation_purchase_res[ i ], insert_value_transaction );
		// 구매발주확정서;
		doc_transaction_purchase_order_purchase_res = create_transaction_purchase_order_purchase_res( doc_transaction_entity.cd, doc_transaction_purchase_order_purchase_req, insert_value_transaction );
	}
...
/**
 * 판매가견적 n회, 구매가견적 nBrand n회, 판매견적1회, 구매견적 nBrand, 판매발주&구매발주, 입고 n회;
 */
var main_depth4_n_nBrand = function( n )
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
	...
	// 구매발주확정서로부터 정보를 가지고 옴;
	var transaction_purchase_order_purchase_res = [];
	db.getSiblingDB( "transaction_purchase_order_purchase" ).getCollection( "transaction_purchase_order_purchase_res" ).find({
		cd$transaction_entity : doc_transaction_entity.cd
	}).sort({ _id : 1 }).forEach(function( doc ){
		transaction_purchase_order_purchase_res.push( doc );
	});
	i=0, iLen=transaction_purchase_order_purchase_res.length;
	var doc_transaction_stock_wearing_req = null;
	var doc_transaction_packing_list_wearing = null;
	var doc_transaction_inspection_report_wearing = null;
	var doc_transaction_stock_wearing_res = null;
	for( ; i<iLen; ++i )
	{
		// 입고지시서;
		doc_transaction_stock_wearing_req = create_transaction_stock_wearing_req( doc_transaction_entity.cd, transaction_purchase_order_purchase_res[ i ], insert_value_transaction );

		// 패킹리스트;
		doc_transaction_packing_list_wearing = create_transaction_packing_list_wearing( doc_transaction_entity.cd, doc_transaction_stock_wearing_req, insert_value_transaction );

		// 검수/검품보고서;
		doc_transaction_inspection_report_wearing = create_transaction_inspection_report_wearing( doc_transaction_entity.cd, transaction_purchase_order_purchase_res[ i ], doc_transaction_stock_wearing_req, insert_value_transaction );

		// 입고증;
		doc_transaction_stock_wearing_res = create_transaction_stock_wearing_res( doc_transaction_entity.cd, doc_transaction_stock_wearing_req, doc_transaction_packing_list_wearing, doc_transaction_inspection_report_wearing, insert_value_transaction );

		// 재고;
		create_stock_entities( doc_transaction_entity.cd, doc_transaction_stock_wearing_res, transaction_purchase_order_purchase_res[ i ], doc_transaction_inspection_report_wearing, insert_value_transaction );
	}
...
/**
 * 판매가견적 n회, 구매가견적 nBrand n회, 판매견적1회, 구매견적 nBrand, 판매발주&구매발주, 입고 n회, 출고 n회;
 */
var main_depth5_n_nBrand = function( n )
{
	// 0.거래 생성 --------------------------------------------------;
	var doc_transaction_entity = create_transaction_entity( insert_value_transaction );
	...
	// 출고;
	i=0, iLen=n;
	var doc_transaction_stock_release_req = null;
	var doc_transaction_inspection_report_release = null;
	var doc_transaction_packing_list_release = null;
	var doc_transaction_stock_release_res = null;
	for( ; i<iLen; ++i )
	{
		// 출고지시서;
		doc_transaction_stock_release_req = create_transaction_stock_release_req( doc_transaction_entity.cd, doc_transaction_purchase_order_sale_res, insert_value_transaction );
		// 검수/검품보고서;
		doc_transaction_inspection_report_release = create_transaction_inspection_report_release( doc_transaction_entity.cd, doc_transaction_stock_release_req, insert_value_transaction );
		// 패킹리스트;
		doc_transaction_packing_list_release = create_transaction_packing_list_release( doc_transaction_entity.cd, doc_transaction_inspection_report_release, insert_value_transaction );
		// 출고증;
		doc_transaction_stock_release_res = create_transaction_stock_release_res( doc_transaction_entity.cd, doc_transaction_packing_list_release, doc_transaction_purchase_order_sale_res, doc_transaction_inspection_report_release, insert_value_transaction );
	}
